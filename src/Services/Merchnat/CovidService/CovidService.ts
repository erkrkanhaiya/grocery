import { COVID } from '../../../Constants/ResponseTypes';
import { tokenfetcher } from '../../../Helpers';

class CovidService {
  queryKeys = {
    fetch: 'fetchcovid',
    fetchMany: 'fetchManycovid',
  };

  fetch = async (Id: string) => {
    return tokenfetcher({
      url: 'covid-form-details?covid_form_id=' + Id,
    });
  };

  fetchMany = async ({ pageParam = 1 }) => {
    return tokenfetcher({
      url: 'all-covid-forms?page=' + pageParam,
    });
  };

  extract = (responseData: any): COVID => {
    return responseData?.data;
  };

  extractMany = (pages: Array<any> = []): Array<COVID> => {
  ...
  add = (data: any) => {
    return tokenfetcher({
      url: 'create-covid-form',
      data,
      method: 'POST',
    });
  };

  deleteCovidform = (data: any) => {
    return tokenfetcher({
      url: 'delete-covid-form',
      data,
      method: 'POST',
    });
  };
